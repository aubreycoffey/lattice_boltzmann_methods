...
plot_interval = 100;            % How many timesteps for the next plot update
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FLOW PARAMETERS
Re      = 300;                      % Reynolds number (Main flow parameter)0000
uMax    = 0.08;                      % Maximal velocity at channel inflow
%re=u*l/v, where l is diameter of cylinder
%v=u*l/re=u*(cyl_r*2)/re
nu      = (uMax*(Nx))/Re;  % kinematic viscosity (is chosen such that the given Raynolds number is achieved) 
%3? +1/2= ?, ?=1/omega
...
ux     = zeros(Ny,Nx);    % x-veloxity according to Poiseuille 
ux(Ny,:) = uMax;
...
for t_step = 1:Nt
    ...
    % MACROSCOPIC (DIRICHLET) BOUNDARY CONDITIONS FOR THE FLOW
        ...
        % Inlet: Poiseuille profile for u (rho is set to a matching value)
            ...
            %%% Exercise g)                                                 %%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%            
            rho(:,[2:Ny-1],inlet) = ((1+uy(:,[2:Ny-1],inlet)).^(-1)).*(fIn(1,[2:Ny-1],inlet)+fIn(2,[2:Ny-1],inlet)+fIn(4,[2:Ny-1],inlet)+(2*(fIn(3,[2:Ny-1],inlet)+fIn(7,[2:Ny-1],inlet)+fIn(6,[2:Ny-1],inlet))));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


        % Outlet: Constant pressure (P = rho*R*T) hence we can also use constant density rho = 1 (u is set to a matching value)   
            rho(:,[2:Ny-1],outlet) = ((1+uy(:,[2:Ny-1],outlet)).^(-1)).*(fIn(1,[2:Ny-1],outlet)+fIn(2,[2:Ny-1],outlet)+fIn(4,[2:Ny-1],outlet)+(2*(fIn(3,[2:Ny-1],outlet)+fIn(7,[2:Ny-1],outlet)+fIn(6,[2:Ny-1],outlet))));        
